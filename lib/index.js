...
var DemoList = class extends import_coc.BasicList {
  constructor(nvim) {
    super(nvim);
    this.name = "demo_list";
    this.description = "CocList for coc-grain";
  ...
  async loadItems(context) {
    return [
      {
        label: "coc-grain list item 1",
        data: { name: "list item 1" }
      },
      {
        label: "coc-grain list item 2",
...
async function activate(context) {
  ...
  const client = new import_coc2.LanguageClient("coc-grain", "coc-grain", serverOtpions, clientOptions);
  context.subscriptions.push(import_coc2.services.registLanguageClient(client));
  import_coc2.window.showMessage(`coc-grain works!`);
  context.subscriptions.push(
    import_coc2.commands.registerCommand("coc-grain.Command", async () => {
      import_coc2.window.showMessage(`coc-grain Commands works!`);
    }),
    import_coc2.listManager.registerList(new DemoList(import_coc2.workspace.nvim)),
    import_coc2.sources.createSource({
      name: "coc-grain completion source",
      doComplete: async () => {
